...
internal sealed class WatchAgent : IDisposable {
    ...
    /// </summary>
    /// <param name="inputPath">監視する入力ファイルのパス</param>
    /// <param name="outPath">出力ファイルのパス</param>
    /// <param name="debounce">デバウンス時間</param>
    public WatchAgent(string inputPath, string outPath, TimeSpan debounce)
    {
        _inputFullPath = Path.GetFullPath(inputPath);
        _outPath = outPath;
        _debounce = debounce;

        var dir = Path.GetDirectoryName(_inputFullPath)!;
        var file = Path.GetFileName(_inputFullPath);
    ...
    private async Task RunOnceSerializedAsync()
    {
        ...
        try
        {
            ...
            for (var i = 0; i < MAX_TRY; i++)
            {
                try
                {
                    if (!File.Exists(_inputFullPath))
                    {
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] plan.md missing (deleted?)");
                        return;
                    }
