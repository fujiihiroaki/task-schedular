...
internal static class MarkdownRenderer {
    ...
    /// <param name="sourcePath">元のファイルパス</param>
    /// <returns>Markdown形式のタスクリスト文字列</returns>
    public static string Render(List<TaskItem> ranked, DateTime today, string sourcePath)
    {
        ...
        static DateTime? EffectiveDate(TaskItem t)
        {
    ...
    static void _WriteSection(StringBuilder sb, string title, List<TaskItem> tasks, bool dedupe = false)
    {
        ...
        if (tasks.Count == 0)
        {
            sb.AppendLine("_（なし）_");
            sb.AppendLine();
            return;
        }

        IEnumerable<TaskItem> list = tasks;
        if (dedupe)
            list = tasks.GroupBy(t => t.Id, StringComparer.OrdinalIgnoreCase).Select(g => g.First());
        ...
        foreach (var t in list)
        {
            ...
            sb.AppendLine($"  - score: `{t.Score:0}` / reason: `{t.Reason}`");
        }
