...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        // Commands:
        //  - once  <plan.md> [--out prioritized.md]
        //  - watch <plan.md> [--out prioritized.md] [--debounce-ms 800]
        ...
        if (command is "watch")
        {
            ...
            // 初回生成
            _SafeRun(input, outPath);

            // 常駐監視
            using var agent = new WatchAgent(input, outPath, TimeSpan.FromMilliseconds(debounceMs));
            agent.Start();

            Console.WriteLine($"Watching: {Path.GetFullPath(input)}");
    ...
    private static void _PrintUsage()
    {
        ...
        Console.WriteLine("  dotnet run -- watch <plan.md> [--out prioritized.md] [--debounce-ms 1200]");
    }
    ...
    /// <summary>
    /// タスク処理を安全に実行します。例外をキャッチしてエラーメッセージを出力します。
    /// </summary>
