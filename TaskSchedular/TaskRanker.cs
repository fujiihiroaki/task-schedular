...
internal static class TaskRanker
{
    ...
    /// </summary>
    /// <param name="tasks">ランク付け対象のタスクリスト</param>
    /// <param name="today">基準日（通常は現在日付）</param>
    /// <returns>スコア降順でソートされたアクティブタスクのリスト</returns>
    public static List<TaskItem> Rank(List<TaskItem> tasks, DateTime today)
    {
        var active = tasks.Where(t => !t.IsDone).ToList();

        foreach (var t in active)
        {
            var (score, reason) = _Score(t, today);
            t.Score = score;
            t.Reason = reason;
        }
    ...
    private static (double score, string reason) _Score(TaskItem t, DateTime today)
    {
        ...
        if (effective is { } d)
        {
            ...
            score += boost;
            var label = t.Start is not null ? "start" : "due";
            reasons.Add($"{label}:{d:yyyy-MM-dd}(+{boost:0})");
        }
        else
        {
            score += 5;
            reasons.Add("no start/due(+5)");
        }
        ...
        if (t.Estimate is { } est)
        {
            ...
            score += boost;
            if (boost > 0) reasons.Add($"est:{_FormatEst(est)}(+{boost:0})");
        }

        // Tag heuristics (customize)
        if (t.Tags.Contains("urgent"))
        {
            score += 50;
        ...
        {
            score += 35;
            reasons.Add("tag:blocking(+35)");
        }

        // Scheduled under today's section
        if (t.SectionDate == today)
        {
            score += 15;
            reasons.Add("today section(+15)");
        }
