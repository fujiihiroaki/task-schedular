...
internal static class Inference {
    ...
    public static void ApplyAutoStart(TaskItem t)
    {
        ...
        // pace があれば lead を上書き（ユーザーに易しい）
        var paceLead = _PaceToLead(t.Pace);
        if (paceLead is not null)
            t.Lead = paceLead;

        // lead 未指定なら tag/キーワードから推定
    ...
    private static TimeSpan _GuessLeadFromTagsOrTitle(TaskItem t)
    {
        ...
        if (t.Tags.Contains("tax"))
            return TimeSpan.FromDays(30);

        if (t.Tags.Contains("travel"))
            return TimeSpan.FromDays(14);

        // キーワード（日本語/英語のざっくり判定）
        var title = t.Title;
        if (_ContainsAny(title, "試験", "資格", "exam", "cert", "certification"))
            return TimeSpan.FromDays(90);

        if (_ContainsAny(title, "確定申告", "申告", "tax"))
            return TimeSpan.FromDays(30);
