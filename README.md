...
```bash
dotnet run -- once <入力ファイル.md> [--out <出力ファイル.md>]
```

**例:**
```bash
dotnet run -- once plan.md --out prioritized.md
```

#### 2. ファイル監視モード（watchモード）

```bash
dotnet run -- watch <入力ファイル.md> [--out <出力ファイル.md>] [--debounce-ms <ミリ秒>]
```

**例:**
```bash
# デフォルト設定（デバウンス1200ms）
dotnet run -- watch plan.md

# カスタム設定
dotnet run -- watch plan.md --out tasks.md --debounce-ms 500
```
