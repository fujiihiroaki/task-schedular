...
/// <summary>
///     MarkdownTaskParserクラスのユニットテスト
/// </summary>
public class MarkdownTaskParserTests {
    [Fact]
    public void Parse_EmptyMarkdown_ReturnsEmptyList()
    {
        // Arrange
        var markdown = "";
    ...
    [Fact]
    public void Parse_SingleTask_ParsesCorrectly()
    {
        // Arrange
        var markdown = "- [ ] タスク1";
        ...
        Assert.Equal("タスク1", result[0].Title);
        Assert.False(result[0].IsDone);
    }

    [Fact]
    public void Parse_CompletedTask_MarksAsDone()
    {
        // Arrange
        var markdown = "- [x] 完了したタスク";
    ...
    [Fact]
    public void Parse_TaskWithDueDate_ParsesDueDate()
    {
        // Arrange
        var markdown = "- [ ] レポート提出 due:2026-03-15";
    ...
    [Fact]
    public void Parse_TaskWithMultipleMetadata_ParsesAll()
    {
        // Arrange
        var markdown = "- [ ] タスク due:2026-03-15 p:1 est:2h tag:urgent";
    ...
    [Fact]
    public void Parse_TaskWithPace_SetsCorrectPace()
    {
        // Arrange
...
- [ ] タスク1";
    ...
    [Fact]
    public void Parse_TaskUnderDateHeading_InheritsDueDate()
    {
        // Arrange
...
- [ ] タスク3 est:1d";
        ...
        Assert.Equal(TimeSpan.FromMinutes(30), result[0].Estimate);
        Assert.Equal(TimeSpan.FromHours(2), result[1].Estimate);
        Assert.Equal(TimeSpan.FromHours(8), result[2].Estimate);// 1d = 8h
    }

    [Fact]
    public void Parse_LeadFormats_ParsesCorrectly()
    {
        // Arrange
        var markdown = @"- [ ] タスク1 lead:7d
- [ ] タスク2 lead:2w
- [ ] タスク3 lead:3m
- [ ] タスク4 lead:1y";
        ...
        Assert.Equal(TimeSpan.FromDays(7), result[0].Lead);
        Assert.Equal(TimeSpan.FromDays(14), result[1].Lead);// 2w = 14d
        Assert.Equal(TimeSpan.FromDays(90), result[2].Lead);// 3m = 90d
        Assert.Equal(TimeSpan.FromDays(365), result[3].Lead);// 1y = 365d
    }

    [Fact]
    public void ExtractTaskIds_FindsAllIds()
    {
        // Arrange
        var markdown = @"- [ ] タスク1  <!-- id:abc123 -->
- [ ] タスク2  <!-- id:def456 -->
- [ ] タスク3";

        // Act
        var result = MarkdownTaskParser.ExtractTaskIds(markdown);
    ...
    {
        // Arrange
        var markdown = "- [ ] タスク tag:urgent tag:work tag:high-priority";
        ...
        // Assert
        Assert.Single(result);
        Assert.Equal(3, result[0].Tags.Count);
